...
namespace Legendigital.Framework.Common.Images
{
    public static class ImageProcessing
    {
        ...
        public static byte[][] SplitImageFile(byte[] imgfile)
        {
            List<byte[]> imgArray = new List<byte[]>();

            Image img = Image.FromStream(new MemoryStream(imgfile));
            ...
            for (int i = 0; i < pageCount; i++)
            {
                ...
                imgArray.Add(ms.ToArray());
            }
            return imgArray.ToArray();
        }

        public static int GetTifPageCountByFile(byte[] imgfile)
        {
            return GetTifPageCount(Image.FromStream(new MemoryStream(imgfile)));
        }
