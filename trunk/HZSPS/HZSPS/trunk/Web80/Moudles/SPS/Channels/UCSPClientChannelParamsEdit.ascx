<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="UCSPClientChannelParamsEdit.ascx.cs" Inherits="Legendigital.Common.Web.Moudles.SPS.Channels.UCSPClientChannelParamsEdit" %>
<ext:Window ID="winSPSendClientParamsEdit" runat="server" Icon="ApplicationEdit"
    Title="编辑通道下发参数" Width="400" Height="330" AutoShow="false" Maximizable="true" Modal="true"  ConstrainHeader=true
    ShowOnLoad="false">
    <Body>
        ...
        <ext:Button ID="btnSaveSPSendClientParams" runat="server" Text="编辑" Icon="ApplicationEdit">
            <AjaxEvents>
                <Click Before="if(!#{formPanelSPSendClientParamsEdit}.getForm().isValid()) return false;"
                    OnEvent="btnSaveSPSendClientParams_Click" Success="Ext.MessageBox.alert('操作成功', '成功的编辑了通道下发参数。',callback);function callback(id) { #{storeSPSendClientParams}.reload(); };
