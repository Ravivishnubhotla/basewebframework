<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="UCSPClientCodeSycnParamsAdd.ascx.cs" Inherits="Legendigital.Common.WebApp.Moudles.SPS.Codes.UCSPClientCodeSycnParamsAdd" %>
<ext:Window ID="winSPClientCodeSycnParamsAdd" runat="server" Icon="ApplicationAdd" Title="SPClientCodeSycnParams Add "
    Width="400" Height="270" AutoShow="false" Maximizable="true" Modal="true" Hidden="true" AutoScroll="true"
    ConstrainHeader="true" Resizable="true" Layout="Fit">
    <Content>
        <ext:FormPanel ID="formPanelSPClientCodeSycnParamsAdd" runat="server" Frame="true" Header="false"
    ...
    <Buttons>
        <ext:Button ID="btnSavelSPClientCodeSycnParams" runat="server" Text="Add" Icon="Add">
            <DirectEvents>
                <Click Before="if(!#{formPanelSPClientCodeSycnParamsAdd}.getForm().isValid()) return false;" OnEvent="btnSaveSPClientCodeSycnParams_Click"
                    Success="Ext.MessageBox.alert('Operation successful', 'Add a record success' ,callback);function callback(id) {#{formPanelSPClientCodeSycnParamsAdd}.getForm().reset();#{storeSPClientCodeSycnParams}.reload(); };
