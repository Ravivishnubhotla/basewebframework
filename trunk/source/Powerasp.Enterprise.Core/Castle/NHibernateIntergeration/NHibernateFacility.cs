...
using NHibernate;
using Powerasp.Enterprise.Core.Castle.NHibernateIntergeration;
...
namespace Powerasp.Enterprise.Core.Castle.NHibernateIntergeration
{
    ...
    /// objects:
    /// <list type="bullet">
    ///   <item><description>NHibernate.Cfg.HttpCompressionModuleConfigurationSection</description></item>
    ...
    public class NHibernateFacility : AbstractFacility
    {
        ...
        protected void RegisterSessionStore()
        {
            ...
            if (customStore != null)
            {
                ...
                {
                    throw new ConfigurationErrorsException("The specified customStore does " + 
        ...
        protected void ApplyConfigurationSettings(NHibernate.Cfg.Configuration cfg, IConfiguration facilityConfig)
        {
            ...
            foreach(IConfiguration item in facilityConfig.Children)
            {
                ...
                String value = item.Value.Replace("$AppRootPath", appRootPath);

                cfg.SetProperty(key, value);
            }
        }

        protected void RegisterResources(NHibernate.Cfg.Configuration cfg, IConfiguration facilityConfig)
        {
            if (facilityConfig == null) return;
        ...
        protected void RegisterAssemblies(NHibernate.Cfg.Configuration cfg, IConfiguration facilityConfig)
        {
            if (facilityConfig == null) return;
        ...
        private void AssertHasAtLeastOneFactoryConfigured()
        {
            ...
            {
                throw new ConfigurationErrorsException("You need to configure at least one factory to use the NHibernateFacility");
            }
        }

        /// <summary>
        /// 检查是否存在NHibernateFacility配置节
        /// </summary>
        private void AssertHasConfig()
        {
            if (FacilityConfig == null)
            {
                throw new ConfigurationErrorsException("The NHibernateFacility requires an external configuration");
            }
        }
