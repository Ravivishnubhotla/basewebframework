...
<ext:Window ID="winSPUperAdd" runat="server" Icon="ApplicationAdd" Title="新建SPUper"
        ...
        <ext:Button ID="btnSaveSPUper" runat="server" Text="添加" Icon="Add">
            <AjaxEvents>
                <Click Before="if(!#{formPanelSPUperAdd}.getForm().isValid()) return false;" OnEvent="btnSaveSPUper_Click"
                    Success="Ext.MessageBox.alert('操作成功', '成功的添加了系统应用。',callback);function callback(id) {#{formPanelSPUperAdd}.getForm().reset();#{storeSPUper}.reload(); };
