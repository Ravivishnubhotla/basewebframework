<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="UCSPClientChannelParamsAdd.ascx.cs" Inherits="Legendigital.Common.Web.Moudles.SPS.Channels.UCSPClientChannelParamsAdd" %>
<ext:Window ID="winSPSendClientParamsAdd" runat="server" Icon="ApplicationAdd" Title="新建通道下发参数"  ConstrainHeader=true
                ...
                <Body>
                    <ext:FormLayout ID="FormLayoutSPSendClientParams" runat="server" LabelSeparator=":"
                        ...
                        <Anchors>
                            ...
                            <ext:Anchor Horizontal="95%">
                                <ext:Label ID="lblClientName" runat="server" FieldLabel="所属通道" AllowBlank="True" />
                            </ext:Anchor>
                            <ext:Anchor Horizontal="95%">
        ...
        <ext:Button ID="btnSaveSPSendClientParams" runat="server" Text="添加" Icon="Add">
            <AjaxEvents>
                <Click Before="if(!#{formPanelSPSendClientParamsAdd}.getForm().isValid()) return false;"
                    OnEvent="btnSaveSPSendClientParams_Click" Success="Ext.MessageBox.alert('操作成功', '成功的添加了通道下发参数。',callback);function callback(id) {#{formPanelSPSendClientParamsAdd}.getForm().reset();#{storeSPSendClientParams}.reload(); };
" Failure="Ext.Msg.alert('操作失败', result.errorMessage);">
        ...
        <ext:Button ID="btnCancelSPSendClientParams" runat="server" Text="取消" Icon="Cancel">
            <Listeners>
                <Click Handler="#{winSPSendClientParamsAdd}.hide();" />
            </Listeners>
        </ext:Button>
    </Buttons>
</ext:Window>
