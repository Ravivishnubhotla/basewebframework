    ...
    public partial class SPChannelWrapper
    {
        ...
        public string InterfaceUrl
        {
            get
            {
                ...
                string handle = "Recieved.ashx";

                if (this.FuzzyCommand.ToLower().EndsWith("ivr"))
                {
                    handle = "IVRChannel.aspx";
                }
                else if (this.FuzzyCommand.ToLower().EndsWith("cspc"))
                {
                }

                if (context.Request.Url.Port == 80)
                    return string.Format("{0}://{1}/SPSInterface/{2}" + handle, context.Request.Url.Scheme,
                                         context.Request.Url.Host, FuzzyCommand);

                return string.Format("{0}://{1}:{2}/SPSInterface/{3}" + handle, context.Request.Url.Scheme,
