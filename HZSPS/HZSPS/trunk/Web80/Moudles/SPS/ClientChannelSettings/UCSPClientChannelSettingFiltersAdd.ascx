<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="UCSPClientChannelSettingFiltersAdd.ascx.cs"
    Inherits="Legendigital.Common.Web.Moudles.SPS.ClientChannelSettings.UCSPClientChannelSettingFiltersAdd" %>
<ext:Window ID="winSPClientChannelSettingFiltersAdd" runat="server" Icon="ApplicationAdd"
    Title="新建SPClientChannelSettingFilters" Width="390" Height="120" AutoShow="false"
    Maximizable="true" Modal="true" ShowOnLoad="false">
    <Body>
        <ext:FitLayout ID="fitLayoutMain" runat="server">
            <ext:FormPanel ID="formPanelSPClientChannelSettingFiltersAdd" runat="server" Frame="true"
...
<ext:ComboBox ID="cmbFilterProvince" Editable="true" runat="server" FieldLabel="过滤省份" AllowBlank="false" TriggerAction="All">
                                    ...
                                    </ext:ComboBox></ext:Anchor>
                            <ext:Anchor Horizontal="95%">
                                <ext:Hidden ID="txtClientChannelSettingID" runat="server" FieldLabel="ClientChannelSettingID"
                                    AllowBlank="True" />
                            </ext:Anchor>
                        </Anchors>
                    </ext:FormLayout>
    ...
    <Buttons>
        <ext:Button ID="btnSaveSPClientChannelSettingFilters" runat="server" Text="添加" Icon="Add">
            <AjaxEvents>
                <Click Before="if(!#{formPanelSPClientChannelSettingFiltersAdd}.getForm().isValid()) return false;"
                    OnEvent="btnSaveSPClientChannelSettingFilters_Click" Success="Ext.MessageBox.alert('操作成功', '成功的添加了指令过滤条件。',callback);function callback(id) {#{formPanelSPClientChannelSettingFiltersAdd}.getForm().reset();#{storeSPClientChannelSettingFilters}.reload(); };
" Failure="Ext.Msg.alert('操作失败', result.errorMessage);">
        ...
        <ext:Button ID="btnCancelSPClientChannelSettingFilters" runat="server" Text="取消"
            Icon="Cancel">
            <Listeners>
                <Click Handler="#{winSPClientChannelSettingFiltersAdd}.hide();" />
            </Listeners>
        </ext:Button>
    </Buttons>
</ext:Window>
